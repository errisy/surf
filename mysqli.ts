...
module data {
    export class port {
         ...
         * @param sql
         */
        public statement(sql: string): mysqli_stmt {
            var stmt = this.mySQLi.prepare(sql);
            if (!stmt) die('Statement Error: ' + this.mySQLi.error);
            return stmt;
        }
        public bind(stmt: mysqli_stmt, bindings: string, values: any[]) {
         ...
         * @param def
         */
        public typedResults(stmt: mysqli_stmt, def: ITableDef): any[] {
            if (!stmt.execute()) die('Statement Execute Error: ' + stmt.error);
         ...
         * @param stmt
         */
        public results(stmt: mysqli_stmt): any[][] {
            if (!stmt.execute()) die('Statement Execute Error: ' + stmt.error);
            if (function_exists('mysqli_get_result')) {
                var list: any[] = array();
                var resultMetadata = stmt.result_metadata();
         ...
         * @param stmt
         */
        public countResult(stmt: mysqli_stmt): number {
            if (!stmt.execute()) die('Statement Execute Error: ' + stmt.error);
            var res: any[][] = this.results(stmt);
        ...
        public checkDefTableName(def: ITableDef, pattern: string): boolean {
            var arr: any[];
    ...
    export class sql {
        ...
        public insert_into: string = 'insert into ';
        public lb: string = '(';
        public rb: string = ')';
        public where: string = ' where ';
        public and: string = ' and ';
        public equals: string = ' = ';
        public greater_than: string = ' > ';
        public less_than: string = ' < ';
        public variable: string = '?';
        public drop_table: string = 'drop table ';
        public commajoin(values: string[]): string {
            return implode(', ', values);
        }
        public SelectFromWhere(fields: any[], table: string, condition: string): string {
            if (count(fields) == 0) {
        ...
        public sAnd(a: string, b: string): string {
            return a + ' and ' + b;
        }
        public sOr(a: string, b: string): string {
            return a + ' or ' + b;
        }
        public InsertIntoValues(table: string, fields: any[], values: any[]):string {
            return 'insert into ' + table + ' (' + this.commajoin(fields) + ') ' + ' values (' + this.commajoin(values) + ');'; 
        }
        public selectExcept(def: ITableDef, fields: string[]): QueryDef {
            var qDef = new QueryDef();
            var all: FieldDef[] = def._AllFields;
            qDef.bindings = '';
            qDef.query = 'select ';// + def._TableName + ' set '
            qDef.values = array();
            var cnt = count(all);
            var selected: FieldDef[] = array();
            for (var i: number = 0; i < cnt; i++) {
                var field: FieldDef = all[i];
            ...
            cnt = count(selected);
            for (var i: number = 0; i < cnt; i++) {
                var field: FieldDef = selected[i];
                qDef.query = qDef.query + field.name;
                if (i < cnt - 1) qDef.query = qDef.query + ', '
            }
            qDef.query = qDef.query + ' from ' + def._TableName + ' ';
            return qDef;
        }
        public updateExcept(def: ITableDef, fields: string[], item:any): QueryDef {
            var qDef = new QueryDef();
            var all: FieldDef[] = def._AllFields;
            qDef.bindings = '';
    ...
    export class QueryDef {
        ...
        public query: string;
        public values: any[];
        public addBinding(binding: string, obj: any) {
